...
深空赛博 HUD 风格的充电运营可视化大屏（**济南市**），纯 H5 静态实现（零构建、零后端），打开即用。
...
| 页面 | 说明 |
| --- | --- |
| `index.html` | **全市站点总览大屏**：济南市地图 26 座站点打点（涟漪呼吸灯），能源馈线飞线动画，KPI 翻牌、桩状态环图、全市负荷功率曲线（含预测）、24h 电量/订单趋势、站点排行、实时订单流水、告警监控、跑马灯快报 |
...
- **实时数据脉动**：每 5s 全量 KPI/站点指标随机微增并翻牌滚动；充电中/空闲桩数微扰联动环图；负荷与趋势图当前小时点位抖动；SOC 每 4s 递增；详情页水球利用率每 12s 微扰
- 粒子网络背景

## 技术栈

HTML5 + CSS3 + 原生 JS + [ECharts 5](https://echarts.apache.org/)（本地内置，无 CDN 依赖）。地图数据为阿里 DataV 济南市 GeoJSON（简化处理）。
...
└── js/
    ...
    ├── city.js         # 总览大屏逻辑
    ├── station.js      # 详情大屏逻辑
    ├── map-jinan.js    # 济南 GeoJSON（DataV，简化）
    └── lib/echarts.min.js
```

## 数据说明

所有数据为**前端模拟**（站点档案为确定性伪随机，刷新不跳变；运行期指标每数秒随机脉动模拟实时上报），仅作演示用途。接入真实数据只需替换 `js/data.js` 与各 `setInterval` 推送逻辑。
