...
**amber · 熔金指挥舱**
通栏 KPI + 非对称大地图（四角金括）+ 右侧竖排面板（玫瑰图/负荷基线/区县横条榜/指挥快报）。点击地图站点 → 金主题单站详情（布局实况能量流/SOC、桩监视、功率曲线、本站快报）。

**light · 蓝白极简视界**
左侧导航 + 指标卡网格 + 白卡网格布局，浅色地图、TOP5 进度条、订单滚动表、区县柱图。点击地图站点 → 单站详情（桩位实况网格点击高亮、功率曲线、本站订单、站点信息）。

**eco · 青绿生态网络**
碳足迹进度环（等效植树/替代里程）、绿电构成环、光储充能量流（SVG 贝塞尔 + 粒子流）、绿电负荷曲线、碳中和进度条。点击地图站点 → 绿主题单站详情（布局实况、本站能量流、减碳环、桩监视）。
